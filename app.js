...
function layout(content){
  app.innerHTML = `
    <div class="brand"><div class="brand-title">ੴ Gurdwara Discovery</div><div class="badge">Prototype V0.1</div></div>
    ${content}
  `;
}
...
function renderHome(){
  ...
  layout(`
    <section class="card hero">
      ...
      <p class="small-note">Photos are loaded from Wikipedia where available. Historical text is prototype content and should be formally reviewed before public release.</p>
    </section>
  `);
...
function startGame(){
  ...
  state.roundIndex=0; state.score=0; state.streak=0; state.bestStreak=0; state.selected=null; state.answered=false;
  renderQuestion();
}
...
async function renderQuestion(){
  state.screen='question'; state.selected=null; state.answered=false;
  const round = state.rounds[state.roundIndex];
  await getImage(round.correct);
  ...
  layout(`
    <section class="card">
      ...
      <div class="photo-wrap">
        ...
        <div class="photo-fallback" style="display:${state.imageUrl?'none':'block'}"><strong>Photo unavailable</strong><br><span>Use the location clues and choices for this prototype round.</span></div>
        ${state.imageCredit ? `<div class="photo-credit">${state.imageCredit}</div>`:''}
      </div>
      <div class="question"><h2>Which Gurdwara is this?</h2><p class="small-note">Choose one answer.</p></div>
      <div class="answers">
        ${round.options.map(o=>`<button class="answer" data-id="${o.id}">${escapeHtml(o.name)}<br><span class="small-note">${escapeHtml(o.city)}, ${escapeHtml(o.country)}</span></button>`).join('')}
      </div>
    </section>
  `);
...
function answer(id){
  ...
  state.answered=true; state.selected=id;
  const item=state.rounds[state.roundIndex].correct;
  const correct = id===item.id;
  if(correct){ state.streak++; state.bestStreak=Math.max(state.bestStreak,state.streak); state.score += 100 + Math.max(0,state.streak-1)*20; }
  else { state.streak=0; }
  discovered.add(item.id); localStorage.setItem('gurdwara_discovered', JSON.stringify([...discovered]));
  renderResult(correct, item);
}

function renderResult(correct,item){
  layout(`
    <section class="card">
      <div class="result-head">
        <div><div class="result-mark ${correct?'good':'bad'}">${correct?'✓ Correct':'Not quite'}</div></div>
      ...
      <div class="tag-row">${item.values.map(v=>`<span class="tag">${escapeHtml(v)}</span>`).join('')}<span class="tag">${item.difficulty}</span></div>
      <div class="info-grid">
  ...
  document.getElementById('next').onclick = ()=>{
    ...
    else { state.roundIndex++; renderQuestion(); }
  };
}
...
function renderEnd(){
  ...
  layout(`
    <section class="card hero">
      ...
      <p class="lead">Best streak: <strong>${state.bestStreak}</strong><br>Prototype collection discovered: <strong>${discovered.size}/${data.length}</strong></p>
      <div class="row">
